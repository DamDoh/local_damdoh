...
export default function CourseDetailPage({ params }: { params: { courseId: string }}) {
    ...
    const loadCourse = async () => {
      ...
      try {
        ...
        if ((result.data as any).success) {
          ...
          throw new Error((result.data as any).message || t('knowledgeHub.courses.detail.errorLoad'));
        }
      } catch (err: any) {
  ...
  if (!course) return <div className="p-8">{t('knowledgeHub.courses.detail.notFound')}</div>;
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-8">
          <h2 className="text-2xl font-bold mb-4">{t('knowledgeHub.courses.detail.curriculum')}</h2>
          <ul>
