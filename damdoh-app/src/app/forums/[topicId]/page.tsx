...
const fetchPostAuthors = async (posts: any[]): Promise<Record<string, UserProfile>> => {
    ...
    userSnaps.forEach(userSnap => {
        if (userSnap.exists()) {
            ...
            profiles[userSnap.id] = { id: userSnap.id, displayName: "Unknown User" } as any;
        }
    });
...
export default function TopicPage() {
    ...
    const fetchPosts = useCallback(async (isInitialLoad = false) => {
        ...
        try {
            ...
            toast({
                title: t('forums.topic.errorLoadPostsTitle'),
                description: t('forums.topic.errorLoadPostsDescription'),
                variant: "destructive"
            });
        } finally {
        ...
        const fetchInitialData = async () => {
            ...
            try {
                ...
                if (topicDetails) {
                 ...
                 toast({
                    title: t('forums.topic.errorLoadTopicTitle'),
                    description: t('forums.topic.errorLoadTopicDescription'),
                    variant: "destructive"
                });
            } finally {
                setIsLoading(false);
    ...
    if (!topic) {
        return (
             <div className="container mx-auto max-w-4xl py-8 text-center">
                 ...
                 <h3 className="mt-4 text-lg font-semibold">{t('forums.topic.notFoundTitle')}</h3>
                 <p className="mt-2 text-sm text-muted-foreground">{t('forums.topic.notFoundDescription')}</p>
                    ...
                    <div className="flex justify-end mb-6">
                        <Button asChild>
                            <Link href={`/forums/${topicId}/create-post`}>
                                ...
                                {t('forums.topic.createPostButton')}
                            </Link>
                        </Button>
                    </div>
                    <div className="space-y-4">
                        {posts.length > 0 ? (
                            ...
                            <div className="text-center py-12">
                                ...
                                <h3 className="mt-4 text-lg font-semibold">{t('forums.topic.noPosts')}</h3>
                                <p className="mt-2 text-sm text-muted-foreground">{t('forums.topic.noPostsSuggestion')}</p>
                                <Button asChild className="mt-4">
                                    <Link href={`/forums/${topicId}/create-post`}>
                                        <PlusCircle className="mr-2 h-4 w-4" />
                                        {t('forums.topic.firstPostButton')}
                                    </Link>
                                </Button>
                            </div>
                        )}
                        ...
                        >
                            {isLoadingMore ? (
                                <>
                                    ...
                                    {t('forums.topic.loadingMoreButton')}
                                </>
                            ) : (
                                t('forums.topic.loadMoreButton')
                            )}
                        </Button>
                    </CardFooter>
                )}
