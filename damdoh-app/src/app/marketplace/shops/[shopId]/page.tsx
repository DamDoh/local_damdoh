    ...
    useEffect(() => {
        ...
        const fetchShopData = async () => {
            ...
            try {
                ...
                setError(err.message || t('shopPage.errorLoading'));
            } finally {
                setIsLoading(false);
            }
        };
        ...
        return (
            <Card className="text-center p-8">
                ...
                <Button asChild variant="outline" className="mt-4">
                    <Link href="/marketplace"><ArrowLeft className="mr-2 h-4 w-4" />{t('shopPage.backLink')}</Link>
                </Button>
            </Card>
        );
    }
    ...
    return (
        <div className="space-y-8">
            <Link href="/marketplace" className="inline-flex items-center text-sm text-primary hover:underline">
                <ArrowLeft className="mr-1 h-4 w-4"/> {t('shopPage.backLink')}
            </Link>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <CardDescription>{t('shopPage.ownedBy')} <Link href={`/profiles/${shop.ownerId}`} className="text-primary hover:underline">{owner?.name || t('loading')}</Link></CardDescription>
                            </div>
                        </div>
                         <div className="flex gap-2 w-full sm:w-auto">
                            {isOwner ? (
                                <Button className="w-full sm:w-auto"><Edit className="mr-2 h-4 w-4" />{t('shopPage.editButton')}</Button>
                            ) : (
                                <Button asChild className="w-full sm:w-auto">
                                    <Link href={`/messages?with=${shop.ownerId}`}>
                                        <MessageCircle className="mr-2 h-4 w-4" /> {t('shopPage.contactButton')}
                                    </Link>
                                </Button>
                            )}
                        </div>
            ...
            <div>
                <h2 className="text-2xl font-semibold mb-4">{t('shopPage.listingsTitle', { name: shop.name, count: items.length })}</h2>
                {items.length > 0 ? (
                    ...
                    <Card className="text-center p-12">
                         <CardDescription>{t('shopPage.noListings')}</CardDescription>
                    </Card>
                )}
            </div>
        </div>
