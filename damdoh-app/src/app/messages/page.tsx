...
function MessagingContent() {
    ...
    }, [selectedConversation?.id, messages.length, getMessagesCallable, toast]);
                ...
                {conversationHeaderProfile ? (
                    <>
                        <div className="p-4 border-b flex items-center gap-3 bg-background">
                            ...
                            <Avatar>
                                <AvatarImage src={conversationHeaderProfile.photoURL} data-ai-hint="profile person agriculture" />
                                <AvatarFallback>{conversationHeaderProfile.displayName?.substring(0,2) ?? '??'}</AvatarFallback>
                            </Avatar>
                            <h3 className="font-semibold">{conversationHeaderProfile.displayName}</h3>
                        </div>
                                ...
                                <div className="space-y-4">
                                    {messages.map(msg => (
                                        <div key={msg.id} className={cn(
                                            ...
                                            {msg.senderId !== user.uid && <Avatar className="h-6 w-6"><AvatarImage src={conversationHeaderProfile.photoURL}/><AvatarFallback>{conversationHeaderProfile.displayName?.substring(0,1) ?? '?'}</AvatarFallback></Avatar>}
                                            <div className={cn(
