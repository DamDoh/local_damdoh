...
import { performSearch as performSearchFunction } from '@/lib/db-utils'; 
...
export function UniversalSearchModal({ isOpen, onClose, initialQuery = "" }: UniversalSearchModalProps) {
  ...
  const handleQuerySubmit = useCallback(async (queryToSubmit: string) => {
    ...
    try {
      ...
      toast({
          title: t('universalSearch.errorTitle'),
          description: error.message || t('universalSearch.errorDescription'),
          variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl h-[80vh] flex flex-col p-0">
        <DialogHeader className='p-4 border-b'>
          <DialogTitle className='flex items-center gap-2'><Sparkles className="h-5 w-5 text-primary" />{t('universalSearch.title')}</DialogTitle>
        </DialogHeader>
        ...
        <div className="p-4 border-b bg-background/80">
          <form onSubmit={handleSubmitForm} className="flex items-center gap-2">
            ...
            <Button type="submit" disabled={isLoading || !currentQuery.trim()} size="lg">
              ...
              {t('universalSearch.searchButton')}
            </Button>
          </form>
        </div>
          ...
          <div className='p-4 space-y-3'>
             {aiInterpretation && (
                 <div className="p-3 text-xs text-blue-700 bg-blue-50 dark:bg-blue-900/30 dark:text-blue-300 rounded-md border border-blue-200 dark:border-blue-800">
                    <p><strong>{t('universalSearch.aiInterpretation')}:</strong> {aiInterpretation.interpretationNotes}</p>
                 </div>
            )}
            {isLoading ? (
                ...
                <div className='flex flex-col items-center justify-center h-full text-center text-muted-foreground pt-16'>
                    ...
                    <p>{t('universalSearch.prompt', { appName: APP_NAME })}</p>
                </div>
            )}
          </div>
        </ScrollArea>
