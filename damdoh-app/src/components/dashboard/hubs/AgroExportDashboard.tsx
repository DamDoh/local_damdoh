...
export const AgroExportDashboard = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold mb-6">{t('dashboard.hubs.agroExport.title')}</h1>
                ...
                <Card className="col-span-1 md:col-span-2 flex flex-col">
                     <CardHeader className="pb-2">
                        <CardTitle className="text-base flex items-center gap-2">
                           ...
                           {t('dashboard.hubs.agroExport.docsTitle')}
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="flex-grow space-y-2">
                       {pendingCustomsDocs.map(doc => (
                           <div key={doc.id} className="flex justify-between items-center text-sm p-2 bg-background rounded-md border">
                               <div>
                                   <p className="font-medium">{t('dashboard.hubs.agroExport.toLabel')} {doc.destination}</p>
                                   <p className="text-xs text-muted-foreground">{doc.status}</p>
                               </div>
                               <Button asChild variant="secondary" size="sm">
                                   <Link href={doc.vtiLink}>{t('dashboard.hubs.agroExport.prepareDocsButton')}</Link>
                               </Button>
                           </div>
                       ))}
                    </CardContent>
                ...
                <Card className="flex flex-col">
                     <CardHeader className="pb-2">
                        <CardTitle className="text-base flex items-center gap-2">
                           ...
                           {t('dashboard.hubs.agroExport.shipmentsTitle')}
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="flex-grow space-y-2">
                ...
                <Card className="col-span-1 md:col-span-3">
                     <CardHeader>
                        <CardTitle className="text-base flex items-center gap-2">
                           ...
                           {t('dashboard.hubs.agroExport.alertsTitle')}
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2">
                       {complianceAlerts.map(alert => (
                           <div key={alert.id} className="text-sm p-3 border rounded-lg bg-yellow-50 border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-800">
                               <p>{alert.content}</p>
                               <Link href={alert.actionLink} className="text-xs text-primary hover:underline mt-1">{t('dashboard.hubs.fi.learnMore')}</Link>
                           </div>
                       ))}
                    </CardContent>
                </Card>
