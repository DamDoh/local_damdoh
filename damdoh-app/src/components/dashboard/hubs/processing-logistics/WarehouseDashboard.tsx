...
export const WarehouseDashboard = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold mb-6">{t('dashboard.hubs.warehouse.title')}</h1>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">

                <Card>
                    <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">{t('dashboard.hubs.warehouse.utilizationTitle')}</CardTitle>
                ...
                <Card>
                    <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">{t('dashboard.hubs.warehouse.inventoryTitle')}</CardTitle>
                        <Layers className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{inventoryLevels.totalItems.toLocaleString()}</div>
                        <p className="text-xs text-muted-foreground">{inventoryLevels.itemsNeedingAttention} {t('dashboard.hubs.warehouse.attentionLabel')}</p>
                    </CardContent>
                </Card>
                ...
                <Card className="md:col-span-3">
                    <CardHeader>
                        <CardTitle className="text-base flex items-center gap-2">
                            ...
                            {t('dashboard.hubs.warehouse.alertsTitle')}
                        </CardTitle>
                    </CardHeader>
                        ...
                        {predictiveAlerts.map((alert, index) => (
                            <div key={index} className="flex justify-between items-center text-sm p-2 bg-destructive/10 border border-destructive/20 rounded-lg">
                                ...
                                <Button asChild variant="secondary" size="sm">
                                    <Link href={alert.actionLink}>{t('dashboard.hubs.warehouse.investigateButton')}</Link>
                                </Button>
                            </div>
                        ))}
                    </CardContent>
