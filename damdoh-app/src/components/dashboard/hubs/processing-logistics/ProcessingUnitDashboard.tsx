...
export const ProcessingUnitDashboard = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold mb-6">{t('dashboard.hubs.processing.title')}</h1>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">

                <Card>
                    <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">{t('dashboard.hubs.processing.yieldTitle')}</CardTitle>
                        <Sliders className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{yieldOptimization.currentYield}%</div>
                        <p className="text-xs text-muted-foreground">{t('dashboard.hubs.processing.potentialLabel')}: {yieldOptimization.potentialYield}%</p>
                ...
                <Card>
                    <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">{t('dashboard.hubs.processing.wasteTitle')}</CardTitle>
                ...
                <Card className="lg:row-span-2 flex flex-col">
                     <CardHeader className="pb-2">
                        <CardTitle className="text-base flex items-center gap-2">
                            ...
                            {t('dashboard.hubs.processing.packagingInventoryTitle')}
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="flex-grow space-y-2">
                       {packagingInventory.map((item, index) => (
                           <div key={index} className="text-sm p-2 bg-background rounded-md border">
                               ...
                               <p className="text-xs">{t('dashboard.hubs.processing.inStockLabel')}: {item.unitsInStock.toLocaleString()}</p>
                               <p className="text-xs text-muted-foreground">{t('dashboard.hubs.processing.reorderLabel')}: {item.reorderLevel.toLocaleString()}</p>
                           </div>
                       ))}
                    </CardContent>
                     <CardFooter>
                        <Button asChild variant="outline" size="sm" className="w-full">
                            <Link href="/marketplace?category=packaging-solutions">{t('dashboard.hubs.processing.sourceButton')}</Link>
                        </Button>
                    </CardFooter>
                </Card>
                 ...
                 <Card className="md:col-span-2">
                    <CardHeader>
                        <CardTitle className="text-base flex items-center gap-2">
                            <Package />
                            {t('dashboard.hubs.processing.inventoryTitle')}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-right">{t('dashboard.hubs.processing.tonsLabel')}</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                ...
                <Card className="md:col-span-2">
                    <CardHeader>
                        <CardTitle className="text-base flex items-center gap-2">
                            <Truck />
                           {t('dashboard.hubs.processing.ordersTitle')}
                        </CardTitle>
                    </CardHeader>
                     <CardContent>
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    <TableHead>{t('dashboard.hubs.processing.tableSupplier')}</TableHead>
                                    <TableHead>{t('dashboard.hubs.processing.tableDeliveryDate')}</TableHead>
                                    <TableHead>{t('dashboard.hubs.processing.tableStatus')}</TableHead>
                                    <TableHead className="text-right">{t('dashboard.hubs.processing.tableAction')}</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                    ...
                                    <TableRow key={order.id}>
                                        ...
                                        <TableCell className="text-right">
                                            <Button asChild variant="ghost" size="sm">
                                                <Link href={order.actionLink}>{t('dashboard.hubs.crowdfunder.viewButton')}</Link>
                                            </Button>
                                        </TableCell>
                                    </TableRow>
                                ))}
