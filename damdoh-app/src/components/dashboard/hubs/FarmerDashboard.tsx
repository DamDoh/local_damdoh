                    ...
                    {recentCrops.length > 0 ? recentCrops.map(crop => (
                        <div key={crop.id} className="p-2 border rounded-md flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground flex items-center gap-1"><CalendarDays className="h-3 w-3" /> Planted on {crop.plantingDate ? format(new Date(crop.plantingDate), "PPP") : 'N/A'}</p>
                            </div>
                    ...
                    {knfBatches.length > 0 ? knfBatches.map(batch => (
                         <div key={batch.id} className="p-2 border rounded-md flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" />Next step {batch.nextStepDate ? formatDistanceToNow(new Date(batch.nextStepDate), { addSuffix: true }) : 'N/A'}</p>
                            </div>
