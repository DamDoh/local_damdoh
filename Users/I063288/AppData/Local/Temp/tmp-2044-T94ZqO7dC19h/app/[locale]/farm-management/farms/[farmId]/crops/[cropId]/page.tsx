...
import { ArrowLeft, Edit, Weight, Droplets, NotebookPen, Sprout, Eye, Package, CheckCircle, Truck, HardHat } from 'lucide-react';
...
const EventPayload = ({ payload, t }: { payload: any, t: any }) => {
    ...
    if (payload.aiAnalysis) {
        const analysis = payload.aiAnalysis as any;
        return (
...
const TraceabilityEventCard = ({ event }: { event: TraceabilityEvent }) => {
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
        const [cropResult, eventsResult, farmResult] = await Promise.all([
            apiCall(`/farm-management/farms/${farmId}/crops/${cropId}`),
            apiCall(`/traceability/events?farmFieldId=${cropId}`),
            apiCall(`/farm-management/farms/${farmId}`)
        ]);
        ...
        const farmData = farmResult as FarmDetails;
        setFarm(farmData);

        setEvents((eventsResult as { events: TraceabilityEvent[] })?.events || []);
