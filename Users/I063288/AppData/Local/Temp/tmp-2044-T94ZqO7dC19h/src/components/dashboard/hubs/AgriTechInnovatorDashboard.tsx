                ...
                {apiKey.status === 'Active' && (
                    <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
                        <DialogTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-8 w-8" title={t('table.revokeKey')} >
                        ...
                        <DialogContent>
                            ...
                            <DialogFooter>
                                <Button variant="outline" onClick={() => setIsConfirmOpen(false)}>{t('revokeDialog.cancel')}</Button>
     ...
     <Dialog open={!!newlyGeneratedKey} onOpenChange={(open) => !open && setNewlyGeneratedKey(null)}>
        <DialogContent>
            ...
            <Alert variant="destructive">
                ...
                <AlertTitle>{t('newKeyModal.warningTitle')}</AlertTitle>
                <p className="text-xs">{t('newKeyModal.warningDescription')}</p>
            </Alert>
            <div className="p-4 bg-muted rounded-md font-mono break-all text-sm relative">
                {newlyGeneratedKey?.key}
                <Button variant="ghost" size="icon" className="absolute top-1/2 right-2 -translate-y-1/2 h-7 w-7" onClick={handleCopy}>
