...
export const onArticleWriteTranslate = functions.firestore
  ...
  .onWrite(async (change, context) => {
    ...
    if (!sourceLang) {
        console.log(`No significant content change detected in a source language for article ${articleId}. No translation action needed.`);
        return null;
    }
...
export const createKnowledgeArticle = functions.https.onCall(
  async (data, context) => {
    ...
    if (!title_en && !title_km) {
        throw new functions.https.HttpsError("invalid-argument", "error.article.titleRequired");
    }
     ...
     if (!excerpt_en && !excerpt_km) {
        throw new functions.https.HttpsError("invalid-argument", "error.article.excerptRequired");
    }
