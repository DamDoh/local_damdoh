...
export const onSourceDocumentWriteIndex = functions.firestore
    ...
    .onWrite(async (change, context) => {
      ...
      if (!indexableCollections.includes(collectionId)) {
        console.log(
            `Collection ${collectionId} is not configured for search indexing.`,
        );
        return null;
      }
      ...
      if (!newData) {
        console.log(
            `Document deleted in ${collectionId}/${documentId}. Removing from search index.`,
        );
      ...
      let titleEn = "";
      const titleLocal: {[key: string]: string} = {};
      let descriptionEn = "";
      const descriptionLocal: {[key: string]: string} = {};
      let tags: string[] = [];
      ...
      console.log(
          `Extracting data for indexing from ${collectionId}/${documentId} (placeholder)...`,
      );
      ...
      try {
        ...
        console.log(
            `Indexed ${collectionId}/${documentId} with ID ${indexItemRef.id}.`,
        );
        return null;
      } catch (error) {
        console.error(
            `Error indexing document ${collectionId}/${documentId}:`,
            error,
        );
        return null;
      }
...
export const performSearch = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
        "unauthenticated",
        "User must be authenticated to perform a search.",
    );
  }
  ...
  if (
    ...
    throw new functions.https.HttpsError(
        "invalid-argument",
        "Pagination must be an object with valid limit and offset.",
    );
  }

  console.log(
      `Performing search for user ${callerUid} with query: "${query}" and filters: ${JSON.stringify(
          filters,
      )}`,
  );
  ...
  try {
    ...
    const searchResults = searchResultsSnapshot.docs.map((doc) => {
      ...
      const title = docData.title_local?.[userLanguage] || docData.title_en || "";
      const description =
        docData.description_local?.[userLanguage] || docData.description_en || "";
    ...
    console.error(
        `Error performing search for user ${callerUid} with query "${query}":`,
        error,
    );
