...
export const assessRiskForPolicy = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        if (!policyAfter || (change.before.exists && change.after.data()?.updatedAt?.isEqual(change.before.data()?.updatedAt))) {
            return null;
        }
        ...
        try {
            ...
            const insuredAssetsDetails = await Promise.all(
                insuredAssets.map(async (asset) => {
                    ...
                    return assetDoc.exists() ? { type: asset.type, assetId: asset.assetRef.id, details: assetDoc.data() } : null;
                })
            ...
            await newRiskAssessmentRef.set({
                ...
                linkedAssets: insuredAssets.map(asset => ({ type: asset.type, assetRef: asset.assetRef })),
        ...
        try {
            ...
            const claimVerificationData: { [key: string]: any } = {
                ...
                insuredAssetsDetails: await Promise.all(
                    insuredAssets.map(async asset => (await asset.assetRef.get()).data())
                ),
            };
            ...
            if (claimResult.status === 'approved' && claimResult.payoutAmount !== undefined) {
...
export const triggerParametricPayout = functions.firestore
    ...
    .onCreate(async (snapshot: admin.firestore.DocumentSnapshot, context: functions.EventContext) => {
        ...
        try {
            ...
            const relevantPolicies: admin.firestore.QuerySnapshot = { empty: false, docs: [{ id: 'policy123', data: () => ({ policyholderRef: db.collection('users').doc('userABC'), insuredAssets: [{ assetRef: db.collection('geospatial_assets').doc('geoXYZ') }], parametricThresholds: { rainfall: { threshold: 100, periodHours: 24, payoutPercentage: 50 } }, coverageAmount: 10000, currency: 'USD' }) } as admin.firestore.QueryDocumentSnapshot] } as admin.firestore.QuerySnapshot;
            ...
            for (const policyDoc of relevantPolicies.docs) {
                ...
                if (parametricThresholds.rainfall && weatherReading.rainfall !== undefined) {
