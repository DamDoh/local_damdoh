...
exports.getFarm = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        return Object.assign({ id: farmDoc.id }, farmData);
    }
    catch (error) {
...
exports.getFarmCrops = functions.https.onCall(async (data, context) => {
    ...
    try {
        const cropsSnapshot = await db
            ...
            .where("farmId", "==", farmId)
            .get();
        const crops = cropsSnapshot.docs.map((doc) => (Object.assign({ id: doc.id }, doc.data())));
        return crops;
    }
    catch (error) {
...
exports.getUserKnfBatches = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const batches = batchesSnapshot.docs.map((doc) => {
            ...
            return Object.assign(Object.assign({ id: doc.id }, docData), { startDate: docData.startDate.toDate().toISOString(), nextStepDate: docData.nextStepDate.toDate().toISOString(), createdAt: docData.createdAt.toDate().toISOString() });
        });
        return batches;
    }
    catch (error) {
