...
export default function AiAssistantPage() {
  ...
  const submitQuery = useCallback(async (query: string, imageUri?: string | null) => {
    setIsLoading(true);
    if (imageUri) {
        toast({ title: t('aiAssistant.toast.analyzingImageTitle'), description: t('aiAssistant.toast.analyzingImageDescription') });
    }
    ...
    try {
        ...
        console.error("Error calling farming assistant:", error);
        const errorAssistantMessage: ChatMessage = {
            id: `assistant-error-${Date.now() + 1}`,
            role: 'assistant',
            content: { summary: error.details?.message || t('aiAssistant.errorSummary'), detailedPoints: [], suggestedQueries: [] },
            timestamp: new Date(),
        };
  ...
  const handleSendMessage = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const imageToSend = previewDataUri;
    setInputQuery('');
    clearPreview();
    
    await submitQuery(userQueryContent, imageToSend);
  };
