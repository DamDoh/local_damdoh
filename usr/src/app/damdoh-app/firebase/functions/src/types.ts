...
export interface CertificationBodyDashboardData {
  ...
  standardsMonitoring: [ { standard: string; adherenceRate: number; alerts: number; actionLink: string; } ]
}


export interface ResearcherDashboardData {
  availableDatasets: [ { id: string; name: string; dataType: string; accessLevel: 'Public' | 'Requires Request'; actionLink: string; } ];
  ongoingProjects: [ { id: string; title: string; progress: number; collaborators: string[]; actionLink: string; } ];
  knowledgeHubContributions: [ { id: string; title: string; status: 'Published' | 'Pending Review' | 'Draft'; actionLink: string; } ]
}

export interface AgronomistDashboardData {
    assignedFarmersOverview: [ { id: string; name: string; farmLocation: string; lastConsultation: string; alerts: number; } ];
    pendingConsultationRequests: [ { id: string; farmerName: string; issueSummary: string; requestDate: string; } ];
    knowledgeBaseContributions: [ { id: string; title: string; status: 'Published' | 'Pending Review'; } ]
}


export interface EnergyProviderDashboardData {
  projectLeads: [
    { id: string; entityName: string; location: string; estimatedEnergyNeed: string; status: 'New' | 'Contacted' | 'Proposal Sent' | 'Closed'; actionLink: string; }
  ];
  activeProjects: [
    { id: string; projectName: string; solutionType: string; status: 'In Progress' | 'Completed'; completionDate: string; }
  ];
  impactMetrics: { totalInstallations: number; totalEstimatedCarbonReduction: string; }
}


export interface CrowdfunderDashboardData {
  portfolioOverview: { totalInvested: number; numberOfInvestments: number; estimatedReturns: number; };
  suggestedOpportunities: [ { id: string; projectName: string; category: string; fundingGoal: number; amountRaised: number; actionLink: string; } ];
  recentTransactions: [ { id: string; projectName: string; type: 'Investment' | 'Payout'; amount: number; date: string; } ]
}
