...
async function _internalInitiatePayment(data: any, context?: functions.https.CallableContext) {
    const { orderId, amount, currency, buyerInfo, sellerInfo, description } = data;
...
export const matchFundingOpportunities = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        try {
            ...
            const userScoreDoc = await db.collection('credit_scores').doc(relevantUserId).get(); 
            ...
            const loanProductsSnapshot = await db.collection('loan_products').where('status', '==', 'open').get();
            const grantProgramsSnapshot = await db.collection('grant_programs').where('status', '==', 'open').get();
            const crowdfundingProjectsSnapshot = await db.collection('crowdfunding_projects').where('status', '==', 'open').get();
            ...
            const matchedOpportunities = matchedOpportunitiesResult.matchedOpportunities || [];
             ...
             if (matchedOpportunities.length > 0) {
                console.log(`Matched opportunity IDs: ${matchedOpportunities.map(opp => opp.opportunityId).join(', ')}`);
             }
