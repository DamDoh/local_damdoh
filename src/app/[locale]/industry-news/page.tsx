...
export default function IndustryNewsPage() {
  const t = useTranslations('industryNewsPage');
  ...
  const newsCategories = [t('allCategories'), "Market Trends", "Technology", "Sustainability", "Regulations", "Research"];
            ...
            <Card key={item.id} className="flex flex-col overflow-hidden hover:shadow-xl transition-shadow duration-300">
              ...
              <CardHeader>
                ...
                <CardDescription className="text-xs">
                  {t('sourceLabel')}: {item.source} | {t('publishedLabel')}: {item.date} | {t('categoryLabel')}: {item.category}
                </CardDescription>
              </CardHeader>
              ...
              <CardContent className="pt-2">
                 <Button asChild variant="outline" className="w-full">
                  <Link href={item.link}>{t('readButton')}</Link>
                </Button>
              </CardContent>
            </Card>
          ))}
        ...
        <Card>
          <CardContent>
            <div className="min-h-[300px] flex flex-col items-center justify-center text-center border-2 border-dashed border-muted-foreground/30 rounded-lg p-8">
              ...
              <h3 className="text-xl font-semibold text-muted-foreground mb-2">{t('noNews.title')}</h3>
              <p className="text-muted-foreground max-w-md">
                {t('noNews.description')}
              </p>
            </div>
          </CardContent>
        </Card>
