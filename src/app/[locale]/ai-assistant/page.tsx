...
import { askFarmingAssistant, type FarmingAssistantOutput } from '@/lib/server-actions';
...
export default function AiAssistantPage() {
  ...
  const initialWelcomeMessage: ChatMessage = {
    ...
    role: 'assistant',
    content: {
      summary: t('initialMessage.summary', { appName: APP_NAME }),
      detailedPoints: [],
      suggestedQueries: t.raw('initialMessage.suggestedQueries')
    },
    timestamp: new Date(),
  };
  ...
  const startCamera = async () => {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      try {
        ...
        toast({
          variant: 'destructive',
          title: t('toast.cameraDenied.title'),
          description: t('toast.cameraDenied.description'),
        });
      }
    } else {
      toast({ variant: 'destructive', title: t('toast.cameraNotSupported.title'), description: t('toast.cameraNotSupported.description') });
    }
  };
  ...
  const resetChat = () => {
    ...
    clearPreview();
    toast({
        title: t('toast.chatReset.title'),
        description: t('toast.chatReset.description'),
    });
  }
  ...
  const handleSendMessage = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const newUserMessage: ChatMessage = {
      id: `user-${Date.now()}`,
      role: 'user',
      content: query || (previewDataUri ? t('imageUpload.prompt') : "Empty query"),
    ...
    try {
      if (currentImageToSend) {
        loadingToastId = toast({
          title: t('toast.aiAnalyzing.title'),
          description: t('toast.aiAnalyzing.description'),
          duration: Infinity,
        }).id;
      }
      const aiResponse = await askFarmingAssistant({
        query: query || (currentImageToSend ? t('imageUpload.prompt') : "Empty query"),
      ...
      <Card className="flex-1 flex flex-col overflow-hidden shadow-xl">
        <CardHeader className="border-b flex-row justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" className="h-9 w-9" onClick={resetChat} title={t('resetButton')}>
            ...
            {isCameraOpen && (
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex gap-2">
                    <Button onClick={handleCaptureImage} size="sm"><Camera className="mr-2 h-4 w-4" />{t('captureButton')}</Button>
                    <Button variant="outline" size="sm" onClick={stopCamera}><RefreshCcw className="mr-2 h-4 w-4" />{t('closeCameraButton')}</Button>
                </div>
              </div>
            )}
          </div>
        ...
        { hasCameraPermission === false && !isCameraOpen && (
           <Alert variant="destructive" className="m-4">
              ...
              <AlertTitle>{t('toast.cameraDenied.title')}</AlertTitle>
              <AlertDescription>
                {t('toast.cameraDenied.description')}
              </AlertDescription>
            </Alert>
        )}
        ...
        <CardFooter className="p-2 border-t">
          <form onSubmit={handleSendMessage} className="flex items-center gap-2 w-full">
            <Button variant="ghost" size="icon" type="button" onClick={() => fileInputRef.current?.click()} title={t('uploadButton')} disabled={isLoading || isCameraOpen}>
                <ImageUp className="h-5 w-5" />
            </Button>
            <input type="file" ref={fileInputRef} accept="image/*" onChange={handleFileChange} style={{ display: 'none' }}/>

            <Button variant="ghost" size="icon" type="button" onClick={startCamera} title={t('cameraButton')} disabled={isLoading || isCameraOpen}>
