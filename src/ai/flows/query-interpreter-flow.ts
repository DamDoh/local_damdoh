...
const SuggestedFilterSchema = z.object({
  type: z.string().describe("The type of filter suggested (e.g., 'category', 'listingType', 'locationScope')."),
  value: z.string().describe("The suggested value for the filter (e.g., 'fresh-produce-fruits', 'Product', 'Region').")
});
...
Your task is to analyze the user's raw search query and extract structured information to help make the search smarter.
...
4.  **suggestedFilters**: Based on the query, suggest potential filters an e-commerce platform might use. For example:
    *   If "buy fresh tomatoes" is queried, suggest: \`[{ type: 'listingType', value: 'Product' }, { type: 'category', value: 'fresh-produce-vegetables' }]\`.
...
- interpretationNotes: "User is looking for second-hand tractors specifically within the East African region."
